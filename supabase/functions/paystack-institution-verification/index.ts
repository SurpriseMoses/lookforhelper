...
const PRICING: Record<string, { currency: string; amount: number }> = {
  "South Africa": { currency: "ZAR", amount: 14900 },
  "Nigeria": { currency: "NGN", amount: 1200000 },
  "Kenya": { currency: "KES", amount: 150000 },
  "Ghana": { currency: "GHS", amount: 15000 },
};
const DEFAULT = { currency: "USD", amount: 900 };
...
serve(async (req) => {
  ...
  try {
    ...
    const { data: inst } = await supabase.from("institutions").select("id, country").eq("user_id", user.id).maybeSingle();
    ...
    if (action === "initialize") {
      ...
      const res = await fetch("https://api.paystack.co/transaction/initialize", {
        ...
        body: JSON.stringify({
          ...
          callback_url,
          metadata: { user_id: user.id, institution_id: inst.id, type: "institution_verification" },
        }),
      });
      ...
      await supabase.from("institution_payments").insert({
        ...
        payment_type: "verification", payment_status: "pending", payment_reference: d.data.reference, payment_country: inst.country,
      });
